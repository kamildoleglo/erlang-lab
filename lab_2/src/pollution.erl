...
addStation(Name, {X, Y}, Monitor, error, error) ->
  ...
  Monitor#monitor{stations = maps:put(Name, #station{name = Name, coordinates = {X,Y}}, S),
...
addValue(Name, Date, Type, Value, Monitor, Station, Measurement_type, error) -> %when there is no measurement with given type for given date, then add one
  ...
  Monitor#monitor{stations = maps:update(Name, Station_new, Monitor#monitor.stations)};
...
getDailyMean(Type, Day, Monitor) ->
  ...
  lists:foldl(fun({S, _}, Sum) -> S + Sum end, 0, Sums) / lists:foldl(fun({_, L}, Sum) -> L + Sum end, 0, Sums).
...
getAirQualityIndex(_, {Date, {Hour, _, _}}, _, {ok, Station}) ->
  ...
  lists:max(lists:map(fun({K,V}) -> divide(lists:max(maps:values(V)), K) * 100 end, List));
