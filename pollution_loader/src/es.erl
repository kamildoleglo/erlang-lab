...
%PARSE_SAX
    ...
    fileSax() ->
      {ok, Xml} = file:read_file("src/books.xml"),
      erlsom:parse_sax(
        Xml,
        [],
        fun(Event, Acc) ->
...
escan()->
  erlsom:compile_xsd_file("src/books.xsd")
...
eloadToModel()->
  {ok, Xml} = file:read_file("src/books.xml"),            %otworzenie pliku xml
  {ok, Model} = erlsom:compile_xsd_file("src/books.xsd"), %tworzenie modelu
